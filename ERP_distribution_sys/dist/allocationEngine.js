 ...
 */
function signalForBlocked(blockedReason) {
 ...
 */
function computeSignal(order, chosenBatch, originalAvailableQty, totalScore, today, existingAllocatedBatchIds, allocatedThisRun) {
    ...
    if (remainingQty < order.requestedQty * YELLOW_REMAINING_QTY_RATIO) {
        yellowReasons.push(`批次分配後剩餘庫存（${remainingQty}）不足本次需求量（${order.requestedQty}）的 ${YELLOW_REMAINING_QTY_RATIO} 倍，下一筆訂單可能無貨`);
    }
...
async function runAllocation(input, options = {}) {
    ...
    const existingAllocatedBatchIds = options.existingAllocatedBatchIds ?? new Set();
    ...
    const allocatedThisRun = new Set();
    for (const order of orderedByPriority) {
        ...
        const status = chosenBatch.availableQty < 0
            ...
            : "recommended";
        // 5d. 燈號分流
        const signal = computeSignal(order, chosenBatch, originalAvailableQty, best.totalScore, today, existingAllocatedBatchIds, allocatedThisRun);
