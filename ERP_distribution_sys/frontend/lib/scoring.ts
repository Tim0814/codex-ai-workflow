 ...
 */
export function scoreExpiry(batch: Batch, today: Date): number {
 ...
 * daysUntilDue >= 30 → 0
 */
export function scoreUrgency(order: Order, today: Date): number {
  const msPerDay = 86_400_000;
 ...
 */
export function scoreOrderTime(order: Order, today: Date): number {
  const msPerDay = 86_400_000;
 ...
 */
export function scoreCustomerTier(customer: Customer): number {
 ...
 * 代表可合併出貨，也給予滿分。
 */
export function scoreRegionCluster(
  batch: Batch,
  customer: Customer,
  pendingOrderRegions: Set<string> = new Set(),
): number {
  if (batch.warehouseRegion === customer.region) return 100;
  if (pendingOrderRegions.has(batch.warehouseRegion)) return 100;
  return 0;
}
